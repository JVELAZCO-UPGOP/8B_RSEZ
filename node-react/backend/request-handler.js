...
module.exports = (req, res) => {
    ...
    //3.1.2 dar resouesta inmediata cuando el metodo sea options
    if(metodo === 'options'){
        res.writeHead(200);
        res.end();
        return;
    ...
    req.on('end',()=>{
        ...
        if(rutaLimpia.indexOf("/")> -1){
           var [rutaPricipal, indice] = rutaLimpia.split('/');
        }

        //3.5 ordenar la data del request
        const data = {
            indice,
            ruta: rutaPricipal || rutaLimpia,
            query,
            metodo,
            headers,
            payload: buffer
        ...
        //4. ejecutar handler {manejador} para enviar las respuestas 
